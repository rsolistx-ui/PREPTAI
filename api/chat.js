...
export default async function handler(req, res) {
  ...
  const validModes = ["chat","match","followup","thankyou","mockgen"];
  ...
  // mockgen is a free utility mode — skip all limit checks and usage logging
  if (mode === "mockgen") {
    try {
      const response = await anthropic.messages.create({
        model: "claude-sonnet-4-20250514",
        max_tokens: 400,
        system: buildMockGenPrompt(cleanSector, cleanRole, cleanCompany, cleanJobDesc),
      ...
      console.error("Mockgen error:", error);
      return res.status(500).json({ error: "mockgen_error", message: "Could not generate questions." });
    }
  }
